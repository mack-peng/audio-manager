...
const fileFilter = (req, file, cb) => {
  const allowedTypes = ['audio/mpeg', 'audio/wav', 'audio/mp3', 'audio/webm', 'audio/amr', 'audio/x-m4a'];
...
// 上传录音文件API
app.post('/api/upload', requireAuth, upload.single('recording'), (req, res) => {
  if (!req.file) {
...
app.get('/api/recordings', requireAuth, async (req, res) => {
  try {
    ...
    // 按上传时间排序，最新的在前面
    recordingList.sort((a, b) => new Date(b.uploadedAt) - new Date(a.uploadedAt));
